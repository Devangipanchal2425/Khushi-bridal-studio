...
import { Home, User, Briefcase, Camera, BookOpen, Mail } from 'lucide-react';

export default function BridalApp() {
  const [activeTab, setActiveTab] = useState('about');

  // Local public directory image paths
  const brideImageUrl = "/bride.jpeg";
  const artistImageUrl = "/khushi.jpeg"; // Photo of Khushi Patel
    ...
    <div className="min-h-screen bg-white text-[#333333] flex flex-col md:flex-row font-sans selection:bg-[#D46A83] selection:text-white">
      ...
      <aside className="w-full md:w-64 bg-black text-white md:min-h-screen flex flex-col justify-between p-8 flex-shrink-0 z-40">
        <div className="space-y-12">
          {/* Logo Box */}
          <div className="bg-[#EFA7B3] text-black p-6 text-center shadow-md">
          ...
          <nav className="flex flex-col space-y-5 text-xs font-semibold tracking-[0.25em] uppercase text-left">
            <button 
              ...
              className={`hover:text-[#EFA7B3] transition-colors text-left ${activeTab === 'home' ? 'text-[#EFA7B3]' : 'text-gray-300'}`}
            >
              HOME
            </button>
            <button 
              onClick={() => setActiveTab('about')} 
              className={`hover:text-[#EFA7B3] transition-colors text-left ${activeTab === 'about' ? 'text-[#EFA7B3]' : 'text-gray-300'}`}
            >
              ABOUT
            </button>
            <button 
              onClick={() => setActiveTab('services')} 
              className={`hover:text-[#EFA7B3] transition-colors text-left ${activeTab === 'services' ? 'text-[#EFA7B3]' : 'text-gray-300'}`}
            >
              SERVICES
            </button>
            <button 
              onClick={() => setActiveTab('portfolio')} 
              className={`hover:text-[#EFA7B3] transition-colors text-left ${activeTab === 'portfolio' ? 'text-[#EFA7B3]' : 'text-gray-300'}`}
            >
              PORTFOLIO
            </button>
            <button 
              onClick={() => setActiveTab('contact')} 
              className={`hover:text-[#EFA7B3] transition-colors text-left ${activeTab === 'contact' ? 'text-[#EFA7B3]' : 'text-gray-300'}`}
            >
              CONTACT
            </button>
          </nav>
      ...
      <main className="flex-1 bg-white p-6 md:p-16 overflow-y-auto">
        ...
        {/* ABOUT ME SECTION (Reference Layout Matching) */}
        {activeTab === 'about' && (
          <div className="max-w-6xl mx-auto space-y-12 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
              ...
              <div className="lg:col-span-5">
                <div className="w-full h-[450px] md:h-[550px] bg-gray-100 overflow-hidden shadow-sm">
                  <img 
                    ...
                    onError={(e) => {
                      // Fallback if khushi.jpeg is not uploaded yet
              ...
              {/* Right Column: Title, Social Icons & Description */}
              <div className="lg:col-span-7 space-y-6">
                
                {/* Header Row with Top-Right Social Icons */}
                <div className="flex justify-between items-start">
                  ...
                  {/* Top Right Circular Social Links */}
                ...
                {/* Bio Paragraphs */}
{/* Bio Paragraphs */}
<div className="space-y-5 text-gray-600 text-sm md:text-base leading-relaxed font-light pt-2">
  <p>
    ...
    To me, bridal makeup is far more than a technique—it is an intimate art form of capturing emotions, confidence, and joy. It is an absolute privilege to walk alongside you on your most cherished journey and make your dream bridal look come alive.
                ...
                <div className="pt-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="border border-gray-100 bg-white p-4 text-center rounded-sm hover:border-[#EFA7B3] transition-all flex flex-col items-center justify-center space-y-1">
                      ...
                      <p className="text-[10px] font-serif font-bold text-gray-800">BEST BRIDAL</p>
                      <p className="text-[8px] text-gray-400 uppercase">ARTIST 2024</p>
                    </div>

                    <div className="border border-gray-100 bg-white p-4 text-center rounded-sm hover:border-[#EFA7B3] transition-all flex flex-col items-center justify-center space-y-1">
                      <span className="text-lg">👑</span>
                      <p className="text-[10px] font-serif font-bold text-gray-800">CERTIFIED HD</p>
                      <p className="text-[8px] text-gray-400 uppercase">EXPERT</p>
                    </div>

                    <div className="border border-gray-100 bg-white p-4 text-center rounded-sm hover:border-[#EFA7B3] transition-all flex flex-col items-center justify-center space-y-1">
                      <span className="text-lg">✨</span>
                      <p className="text-[10px] font-serif font-bold text-gray-800">AIRBRUSH</p>
                      <p className="text-[8px] text-gray-400 uppercase">SPECIALIST</p>
                    </div>

                    <div className="border border-gray-100 bg-white p-4 text-center rounded-sm hover:border-[#EFA7B3] transition-all flex flex-col items-center justify-center space-y-1">
                      <span className="text-lg">🏅</span>
                      <p className="text-[10px] font-serif font-bold text-gray-800">500+ BRIDES</p>
                      <p className="text-[8px] text-gray-400 uppercase">TRANSFORMED</p>
                    </div>
                  </div>
                </div>
        ...
        {activeTab === 'home' && (
          <div className="h-full flex flex-col justify-center items-center text-center py-20 space-y-4">
            <h2 className="font-serif text-4xl text-black">Welcome to Khushi Patel Studio</h2>
            <p className="text-gray-500 text-sm max-w-md">Luxury Bridal Makeovers & High-End Beauty Artistry.</p>
            <button onClick={() => setActiveTab('about')} className="bg-black text-white text-xs uppercase tracking-widest px-8 py-3 mt-4 hover:bg-[#EFA7B3] hover:text-black transition-all">
              Explore About Me
            </button>
          </div>
        )}
        ...
        {(activeTab === 'services' || activeTab === 'portfolio' || activeTab === 'contact') && (
          <div className="py-20 text-center space-y-2">
            <h2 className="font-serif text-3xl uppercase tracking-wider">{activeTab} Section</h2>
            <p className="text-gray-400 text-xs">Content loading...</p>
          </div>
        )}

      </main>

      {/* Floating Official WhatsApp Logo Button */}
      <a 
        href={whatsappUrl} 
        target="_blank" 
        rel="noreferrer" 
